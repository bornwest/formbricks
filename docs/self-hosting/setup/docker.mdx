...
1. **Download the Docker Files**
   ...
   curl -fsSL \
     ...
     https://raw.githubusercontent.com/bornwest/formbricks/refs/heads/main/docker/docker-compose.yml
   curl -fsSL \
...
See [AuthZed Operations](/self-hosting/configuration/authzed-operations) before applying a schema, repairing
...
1. Follow the [maintenance upgrade guide](/self-hosting/advanced/v6-maintenance-upgrade): stop all writers and
   controllers, drain in-flight work, then back up configuration, secrets, and databases.
...
5. Validate the reviewed Compose configuration privately and pull the pinned images. Keep writers stopped.
6. Wait for PostgreSQL and SpiceDB, then run the target image's maintenance command. It runs application
   migrations before graph preparation and verification:

   ```bash
   docker compose up -d --wait postgres spicedb
   ...
   [AuthZed Operations](/self-hosting/configuration/authzed-operations#upgrade-an-existing-installation-to-v6).
7. Only after preparation and verification exit `0`, set `FORMBRICKS_AUTHZED_V6_MIGRATION_ACKNOWLEDGED=true` in `.env` and
   run `docker compose up -d`. The one-click updater only checks the already-prepared graph; it does not perform
   online preparation. Keep maintenance active until authorization checks pass.
...
After database changes, restoring the old image alone may not be safe. Remain in maintenance and either fix
forward or restore coordinated backups with the matching v5 configuration. Never delete volumes. See the
rollback procedure in [AuthZed Operations](/self-hosting/configuration/authzed-operations#roll-back-a-v6-upgrade).
...
1. Check the target release's already-prepared authorization graph. If blocked, follow the maintenance
   preparation guide above before proceeding.
...
1. Wait for PostgreSQL and complete application migrations before restarting the updated stack. Stop if a
   command fails; do not restart against a partially migrated database.
...
<Warning>
  **Security (existing installs):** these steps — and the [one-click setup script](/self-hosting/setup/one-click)
  updater — do **not** rewrite your existing
...
- **Missing or empty secrets**: Confirm `.env` contains `BETTER_AUTH_SECRET`, `ENCRYPTION_KEY`, `CRON_SECRET`,
...
<Warning>
  ...
  **If the port was reachable from an untrusted network, also invalidate existing sessions.** Valkey persists
  ...
  `BETTER_AUTH_SECRET` in your `.env`, then recreate the app container so it picks up the change:
  ```bash
  ...
  data, the job queue, and the cache stay intact. (On an instance installed before this variable was
  renamed, the secret may live in your `.env` under the older name, `NEXTAUTH_SECRET`. Replace the value
  of whichever name your `.env` actually defines. Adding the *other* name alongside it does not reliably
  rotate anything: `BETTER_AUTH_SECRET` wins whenever both are set, so a new `NEXTAUTH_SECRET` next to an
  existing `BETTER_AUTH_SECRET` changes nothing and leaves the forged session valid.) Leave
  `ENCRYPTION_KEY` unchanged, and do **not** run a blanket `FLUSHDB` — the same Valkey also holds BullMQ
  job queues and the application cache.
</Warning>
