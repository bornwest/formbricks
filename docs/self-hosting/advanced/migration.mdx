...
Formbricks v6 replaces the legacy authorization evaluator with SpiceDB, maintained by AuthZed. PostgreSQL
remains the source of roles and grants, but SpiceDB is the sole decision engine. There is no runtime fallback,
so an existing installation must prepare its complete authorization graph before serving v6 traffic.

Use the supported source/target pair in the release notes and the
[maintenance upgrade procedure](/self-hosting/advanced/v6-maintenance-upgrade). A self-hosted v5 bridge is not required:

1. Enter maintenance, stop all writers, suspend their controllers, and drain in-flight work. Take recoverable
   backups of the databases, secrets, configuration, and source image versions.
...
3. Run the target-image database migrations while writers remain stopped, never against the still-running v5 app.
4. Run `formbricks-authzed-prepare --backup-confirmed --writers-stopped`; it runs migrations, reconciliation,
   one clean full audit, and a second independent `upgrade check` audit.
...
The update script and Helm upgrade hook verify the prepared graph without repairing it. Do not bypass a failed
gate. Keep maintenance active on failure: after database changes, recovery may require restoring coordinated
backups rather than running `helm rollback`. Pin explicit releases and digests, not floating tags.

The Helm chart and Formbricks image are versioned independently. A chart-only patch can retain the same
`appVersion`; review both pins and the rendered image rather than assuming their version numbers match.

### Changes in Environment Variables

<Note>
  `DEFAULT_ORGANIZATION_ID` and `DEFAULT_ORGANIZATION_ROLE` drive SSO just-in-time provisioning, which is
  part of the Formbricks [Enterprise Edition](/self-hosting/advanced/license). On a community instance
  they have no effect, so there is nothing to rename.
</Note>
...
- **BETTER_AUTH_URL** and **BETTER_AUTH_SECRET** are now the documented names for the auth base URL and
  the session-signing secret. This is a documentation change, not a rename: `NEXTAUTH_URL` and
  `NEXTAUTH_SECRET` keep working exactly as before, so **an existing instance needs no action**. New
  installs configure the `BETTER_AUTH_*` names, which is what the setup guides now show.

  <Warning>
    If you do choose to move to the new names, **copy the existing value across** rather than generating a
    new one. `BETTER_AUTH_SECRET` takes precedence, so setting it to a *different* value signs everyone
    out and invalidates outstanding invite, email-verification and email-change links. Formbricks logs a
    warning at startup when both are set to different values. The secret is no longer rejected for being
    shorter than 32 characters, so a shorter legacy value can be renamed as-is — you will see a startup
    warning recommending a longer one, which you can act on whenever a forced sign-out is acceptable.
  </Warning>

  Formbricks now refuses to start when neither variable has a value, in every environment. Before this
  release that refusal applied only when `NODE_ENV=production`; a development or test instance with
  neither variable set still started, using a fallback value for the signing secret, and only invites
  and verification links failed. So a production instance is already required
  to have one of these set and is unaffected. If a non-production instance of yours set neither,
  generate a secret before upgrading — doing so **signs every current user out**, because their cookies
  were signed with that fallback value rather than with a secret of yours.
...
### Envoy Gateway 1.8.4 CRD Upgrade

The updated Helm chart replaces Envoy Gateway 1.7.1 and Gateway API 1.4.1 with Envoy Gateway 1.8.4 and Gateway
API 1.5.1. Fresh installations receive the new CRDs automatically. Helm does not update CRDs during an ordinary
upgrade, so existing installations using the bundled controller must apply the matching CRDs before upgrading:
...
`--force-conflicts` transfers the existing CRD fields from Helm's field manager to this explicit CRD upgrade.
...
The chart does not install Envoy Gateway's cluster-wide safe-upgrade admission policy by default. Enabling
`envoy.crds.gatewayAPI.safeUpgradePolicy.enabled` denies Gateway API CRD versions older than v1.5.0 across the
whole cluster, including CRDs reconciled by unrelated releases. Enable it only after platform review; a chart
rollback does not remove that policy automatically.

If the platform already manages compatible Gateway API CRDs, do not install another Gateway API bundle. Install
only the Envoy Gateway CRDs as described in the [Kubernetes guide](/self-hosting/setup/kubernetes), set
`envoy.crds.enabled=false`, and configure Flux to skip CRD installation and upgrades. Upgrade the CRDs before the
controller and never downgrade them during rollback.

The controller scheduling values also move from the ineffective `envoy.deployment.envoyGateway.pod` path to
`envoy.deployment.pod`. Move any node selectors, tolerations, topology spread constraints, extra volumes, or
pod annotations that you placed under the old path before upgrading.

The parent chart no longer repeats the bundled controller's default port list. The
`envoy.deployment.ports` override remains supported, so existing port overrides need no migration.
...
<Note>
  The table writes `your-domain.com` as your `WEBAPP_URL`, which is correct for a default install. The
  callback is derived from the same auth base URL Better Auth itself uses, not from `WEBAPP_URL`: that is
  `BETTER_AUTH_URL`, or `NEXTAUTH_URL` when only the legacy name is set, falling back to `WEBAPP_URL` when
  neither is. So if either auth URL names a different origin, the callback lives on **that** origin — register
  that URI at your identity provider. Either way it is unchanged by this upgrade.
</Note>
...
- `BETTER_AUTH_SECRET` — cookie/session signing secret. Falls back to `NEXTAUTH_SECRET` when unset. On
  these releases it must be at least 32 characters when set explicitly. Use the value `NEXTAUTH_SECRET`
  already holds: a *different* one invalidates every session and every outstanding invite and
  verification link.
...
thresholds, use the [rate-limiting guide](/self-hosting/configuration/rate-limiting) as the source of truth.
...
<Tabs>
  <Tab title="One-Click">
    ...
    ```bash
    curl -fsSL -o formbricks/docker-compose.v5.yml https://raw.githubusercontent.com/bornwest/formbricks/refs/heads/main/docker/docker-compose.yml
    ```

    Then compare `formbricks/docker-compose.v5.yml` with your existing `formbricks/docker-compose.yml` and merge
    the v5 additions:

    - set `POSTGRES_PASSWORD` in `formbricks/.env` to the exact password used by the existing PostgreSQL
  ...
  <Tab title="Docker Compose">
    ...
    ```bash
    curl -fsSL -o docker-compose.v5.yml https://raw.githubusercontent.com/bornwest/formbricks/refs/heads/main/docker/docker-compose.yml
    ```
